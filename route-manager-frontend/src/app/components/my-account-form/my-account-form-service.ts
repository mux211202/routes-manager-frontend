...
export class MyAccountFormService {
  ...
  register(userData: any): void {
    ...
    this.http.post(environment.apiUrl + '/api/v1/auth/register', userData, {headers, observe: 'response'})
      .subscribe(
  ...
  login(userData: any): void {
    ...
    this.http.post(environment.apiUrl + '/api/v1/auth/login', userData, {headers, observe: 'response'})
      .subscribe(
        (response: HttpResponse<any>) => {
          const {token} = response.body;
          localStorage.setItem('jwtToken', token);
